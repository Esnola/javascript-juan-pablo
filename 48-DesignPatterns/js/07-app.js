...
restaurApp.funciones = {
    ordenar: id => {
        console.log(`Tu platillo: ${restaurApp.platillos[id].platillo} se esta preparando, tendrás que pagar $${restaurApp.platillos[id].precio} `);
    },
    ...
    mostrarMenu: platillos => {
        ...
        platillos.forEach((platillo, index) => {
            console.table(`${index})  ${platillo.platillo} $ ${platillo.precio}`)
        });
    }
}

restaurApp.funciones.agregarPlatillo('Pastel', 20.32);
restaurApp.funciones.agregarPlatillo('Taco', 18);
